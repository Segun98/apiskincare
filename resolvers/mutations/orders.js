...
module.exports = {
  ...
  async updateOrder(
    _,
    { order_id, transaction_id, delivery_fee, total_price },
    { pool, req }
  ) {
    verifyJwt(req);
    try {
      await pool.query(
        `insert into order_status (order_id, transaction_id, delivery_fee, total_price) values($1,$2,$3,$4)`,
        [order_id, transaction_id, delivery_fee, total_price]
      );
      return {
  ...
  //by customer
  async cancelOrder(_, { order_id, canceled_reason }, { pool, req }) {
    verifyJwt(req);

    // TODO
    // Ensure customers can't cancel a completed order
 ...
 */
    const in_transit = await pool.query(
      `select in_transit from order_status where order_id = $1`,
      [order_id]
    );
    ...
    try {
      await pool.query(
        `update order_status set canceled=$2, canceled_reason= $3 where order_id = $1`,
        [order_id, "true", canceled_reason]
      );
